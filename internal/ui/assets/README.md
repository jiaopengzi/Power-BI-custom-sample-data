...
界面粗体中文字体，用于表头等需要加粗显示的文案（主题 `Font()` 在 `TextStyle.Bold` 时返回本资源）。

- 由同一可变字体固定到 `wght=700`（Bold）后，按与 `NotoSansSC.ttf` **完全一致的码位**子集化而成，保证任意加粗文案都不缺字。
- 许可同 `NotoSansSC.ttf`（OFL 1.1）。约 2.2 MB。

复现（需 Python + fonttools）：

```python
# 1. 读取 NotoSansSC.ttf 的 cmap, 取其全部码位 unicodes
# 2. instantiateVariableFont(vf, {"wght": 700})
# 3. Subsetter().populate(unicodes=unicodes) 后 subset
# 4. 保存为 NotoSansSC-Bold.ttf
```
