...
- **进度可视化与结果回显**: 生成过程实时显示进度与阶段 (含收尾的 "写入 PBIP 工程" / "统计生成结果"), 100% 与结果表格同帧出现, 稍作停留后进度条自动收起; 完成后以表格展示各表行数与大小, 重新打开软件时会自动回显目录中已有的产物统计与事实数据截止日期。
...
- `internal/i18n/i18n.go` 的 `Manager` 提供 `T(key)` / `Tf(key, vars)` 与运行时 `SetLocale`; `stages.*` 键与生成器/控制器发出的阶段标识对齐。
  进度编排: 生成器内部进度由控制器压缩到 88% 以内, 90% (写入 PBIP 工程) 与 95% (统计生成结果) 由控制器上报, 100% 由界面在结果表格渲染完成后设置。
